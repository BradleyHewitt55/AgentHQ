    ...
    Effect.gen(function* () {
      ...
      return WsRpcGroup.of({
        ...
        [WS_METHODS.serverGetUsageLimits]: (_input) =>
          observeRpcEffect(WS_METHODS.serverGetUsageLimits, usageLimits.getState, {
            "rpc.aggregate": "server",
          }),
        [WS_METHODS.serverRefreshUsageLimits]: (input) =>
          observeRpcEffect(WS_METHODS.serverRefreshUsageLimits, usageLimits.refresh(input), {
            "rpc.aggregate": "server",
          }),
        ...
        [WS_METHODS.projectsMkdir]: (input) =>
          observeRpcEffect(
            WS_METHODS.projectsMkdir,
            workspaceFileSystem.makeDirectory(input).pipe(
              Effect.mapError(
                (cause) =>
                  new ProjectMkdirError({
                    cwd: input.cwd,
                    relativePath: input.relativePath,
                    ...projectFileFailureContext(cause),
                    cause,
        ...
        [WS_METHODS.projectsMove]: (input) =>
          observeRpcEffect(
            WS_METHODS.projectsMove,
            workspaceFileSystem.moveEntry(input).pipe(
              Effect.mapError(
                (cause) =>
                  new ProjectMoveError({
                    cwd: input.cwd,
        ...
        [WS_METHODS.projectsDelete]: (input) =>
          observeRpcEffect(
            WS_METHODS.projectsDelete,
        ...
        [WS_METHODS.serverSubscribeUsageLimits]: (_input) =>
          observeRpcStream(
            WS_METHODS.serverSubscribeUsageLimits,
            Stream.unwrap(
